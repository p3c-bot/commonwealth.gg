 ...
 *
 *  What?
 *  -> eCOM div interface. Send ETH here, and then call distribute to give to eCOM holders.
...
contract Divies {
    ...
    HourglassInterface constant eCOM = HourglassInterface(eCOMAddress);
  ...
  //^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
        ...
        public
    {
        ...
        // buy & sell    
        eCOM.buy.value(_bal)(address(0));
        eCOM.sell(eCOM.balanceOf(address(this)));
            
        // setup tracker.  this will be used to tell the loop to stop
        uint256 _tracker = eCOM.dividendsOf(address(this));
    
        // reinvest/sell loop
        while (_tracker >= _stop) 
        {
            // lets burn some tokens to distribute dividends to eCOM holders
            eCOM.reinvest();
            eCOM.sell(eCOM.balanceOf(address(this)));
