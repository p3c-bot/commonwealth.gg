...
$('#infoButton')
    .popup({
        content: "Allow bots to compound your dividends in exchange for a referral bonus. You can manually withdraw at any time, but this must be on to use Compound. For greater control, use Points Pure Interface.",
        position: 'top center'
    });
    ...
    $.getJSON("https://api.commonwealth.gg/price/crop/" + web3.toChecksumAddress(cropAddress), function (data) {
        if (data !== null){
            ...
            $.each(data, function (key, val) {
                ...
                if (color == "red" && key == 'usd7'){
                    if (typeof gtag !== 'undefined'){gtag('event', 'Wallet', {'event_label': 'Usage', 'event_category': 'BalanceDown','value': change});};
                    alertify.error('Balance: <u>' + change + '</u>% down in last 7 days.',5)
                }

                if (color == "green" && key == 'usd7'){
                    if (typeof gtag !== 'undefined'){gtag('event', 'Wallet', {'event_label': 'Usage', 'event_category': 'BalanceUp','value': change});};
                    alertify.success('Balance: <u>' + change + '</u>% up in last 7 days.',7)
                }
            });
...
$('#copyMNButton').on('click', function (){
    ...
    alertify.success('Referral Link Copied', 2)
})
