...
function setPortfolio(cropAddress) {
    $.getJSON("http://api.p3c.io/price/crop/" + web3.toChecksumAddress(cropAddress), function (data) {
        // (New Number - Original Number) ÷ Original Number × 100.
        $('#portfolioButton').show();
        performance = `
        My Holdings Value (USD/ETC)
        <br>
        <b>1 Day</b>: {usd1} / {etc1}
        <br>
        <b>7 Day</b>: {usd7} / {etc7}
        ...
        <br>
        <span class="ui text small eleven converted">Past performance is no guarantee of future results.</span>
        `
        $.each(data, function (key, val) {
            if (key.includes('usd')){
                change = (((myUSDValue - val) / val) * 100).toFixed(0)
            } else {
                change = (((myETCValue - val) / val) * 100).toFixed(0)
            }
