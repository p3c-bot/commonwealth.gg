...
$('#infoButton')
    .popup({
        content: "Allow bots to compound your dividends in exchange for a referral bonus. This enables faster compounding. You can manually withdraw at any time, but this must be on to use Compound. For greater control, use P3C Pure Interface.",
        position: 'top center'
    });
...
function setPortfolio(cropAddress) {
    $.getJSON("https://api.p3c.io/price/crop/" + web3.toChecksumAddress(cropAddress), function (data) {
        if (data !== null){
            ...
            performance = `
            My holdings performance (in USD terms):
            <br>
            <b>Change 1 Day</b>: {usd1}
            <br>
            <b>USD Change 7 Days</b>: {usd7}
            <br>
            <b>USD Change 30 Days</b>: {usd30}
            <br>
