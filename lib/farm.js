...
function getMyCrop(onboard) {
    ...
    if (myCropAddress == 'null' || myCropAddress == null) {
...
function getMyCropTokens() {
    farmContract.myCropTokens.call(function (err, result) {
        if (!err) {
            ...
            myCropTokens = result;
            if (change) {
                $("#myCropTokens").replaceWith("<b id='myCropTokens'>" + numberWithCommas((web3.fromWei(myCropTokens) * .9999).toFixed(0)) + "</b>")
                p3cContract.sellPrice(function (e, r) {
                    let sellPrice = web3.fromWei(r)
                    $('#myETCValue').text((sellPrice * web3.fromWei(myCropTokens)).toFixed(2))
                })
