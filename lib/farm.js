        ...
        farmContract.myCrop.call(function (err, result) {
            ...
            if (onboard == true && (result == '0x0000000000000000000000000000000000000000')) {
                setTimeout(function () {
                    ...
                    To participate you need and Ethereum Classic Wallet (install <a target="_blank" href="https://chrome.google.com/webstore/detail/saturn-wallet/nkddgncdjgjfcddamfgcmfnlhccnimig">here</a>) with ETC in it. Tutorial is <a href="https://www.youtube.com/watch?v=XEDjtJJH0BE&" target="_blank">here.</a>                    
                    </p>
...
function buyFromCrop(amountToBuy, referrer) {
    farmContract.myCrop.call(function (err, cropAddress) {
        ...
        cropAbi.at(cropAddress).buy.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    alertify.success(amountToBuy + " P3C Purchased. Waiting for Blockchain.")
                    playSound('register');
                } else {
                    console.log(error);
                }
...
function reinvestFromCrop(referrer) {
    farmContract.myCrop.call(function (err, cropAddress) {
        cropAbi.at(cropAddress).reinvest.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    alertify.success("Replayed P3C. Waiting for Blockchain.")
                    console.log(result);
                } else {
                    console.log(error);
                }
