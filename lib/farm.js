            ...
            if (onboard == true && (result == '0x0000000000000000000000000000000000000000')) {
                setTimeout(function () {
                    alertify.confirm(
                        ...
                        P3C is an Ethereum Classic app. Read it <a target="_blank" href="https://blockscout.com/etc/mainnet/address/0xde6fb6a5adbe6415cdaf143f8d90eb01883e42ac/contracts">here</a>. By continuing, you accept that the code will run as written, without guarantee of profit.
...
function getMyCropDividends() {
    farmContract.myCropDividends(true).call(function (err, result) {
        if (!err) {
        ...
        cropAbi.at(cropAddress).reinvest.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("Compounded P3C. Waiting for Blockchain.")
                    // playSound('register');
                    console.log(result);
                } else {
                    console.log(error);
