        ...
        cropAbi.at(cropAddress).sell.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success(amountToSell + " COM Sold. Waiting for Blockchain.")
                    // playSound('register');
                    $('#sellAmount').hide();
                } else {
                    console.log(error);
        ...
        cropAbi.at(cropAddress).reinvest.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("Compounded COM. Waiting for Blockchain.")
                    // playSound('register');
                    console.log(result);
                } else {
                    console.log(error);
        ...
        cropAbi.at(cropAddress).transfer.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("Transfering " + amountToTransfer + " COM to " + destination.substring(0, 7) + "...")
                    // playSound('register');
                    console.log(result);
                } else {
                    console.log(error);
