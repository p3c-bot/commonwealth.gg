            ...
            if (onboard == true && (result == '0x0000000000000000000000000000000000000000')) {
                ...
                if (result !== '0x0000000000000000000000000000000000000000') {
                    ...
                    if (getETCWallet == false){
                        alertify.success('Turn on ETC Mode')
                        getETCWallet = true;
                    }
...
function activateUI(cropAddress) {
    ...
    $("#masternodeLink").replaceWith('<a id="masternodeLink" href="/?ref=' + myCropAddress + '"commonwealth.gg/index.html?ref=' + myCropAddress + '</a>')
    $("#copyMNButton").attr("data-clipboard-text", 'commonwealth.gg/index.html?ref=' + myCropAddress);
        ...
        cropAbi.at(cropAddress).sell.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success(amountToSell + " Points Sold. Funds are deposited in wallet as ETC.")
                    // playSound('register');
                    $('#sellAmount').hide();
                } else {
                    console.log(error);
        ...
        cropAbi.at(cropAddress).reinvest.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("Compounded Points. Waiting for Blockchain.")
                    // playSound('register');
                    console.log(result);
                } else {
                    console.log(error);
        ...
        cropAbi.at(cropAddress).transfer.sendTransaction(
            ...
            function (error, result) { //get callback from function which is your transaction key
                if (!error) {
                    ...
                    alertify.success("Transfering " + amountToTransfer + " Points to " + destination.substring(0, 7) + "...")
                    // playSound('register');
                    console.log(result);
                } else {
                    console.log(error);
