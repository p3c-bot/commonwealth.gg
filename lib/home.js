...
function setStats() {
    $.getJSON("https://api.p3c.io/airdrop/info", function (data) {
		...
		SupplyPercentage = (data.P3CSupply / 204000000).toFixed(4) * 100 + "%"
		...
		$('#createdStats').popup({
            html: 'P3C tokens can <b>only</b> be created by sending Ethereum Classic (ETC) to the contract. There is no central authority that can inflate P3C.',
            position: 'bottom center'
		});
		$('#priceStats').popup({
            html: 'Current price of a new P3C token. All tokens are denominated in ETC.',
            position: 'bottom center'
		});
		$('#sizeStats').popup({
            html: 'P3C uses a <b>bonding curve</b> algorithm similar to Bancor or Uniswap to provide liquidity to all holders.',
            position: 'bottom center'
        });
	});
}